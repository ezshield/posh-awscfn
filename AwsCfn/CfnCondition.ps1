...
#>
function Add-CfnCondition {
    param(
        [Parameter(Mandatory,Position=0)]
        [string]$ConditionName,
        [Parameter(Mandatory,Position=1)]
        [object]$ConditionFunction
    )

    $tConditions = [System.Collections.IDictionary](Get-Variable -Name "tConditions" -ValueOnly)
    if (-not $tConditions) {
        throw "Template Conditions collection is not in scope"
    }
    
    if ($tConditions.Contains($ConditionName)) {
        throw "Duplicate Condition name [$ConditionName]"
    }
    
    $tConditions[$ConditionName] = $ConditionFunction
}
Set-Alias -Name Condition -Value Add-CfnCondition
